...
> **补充说明（text）的用法**：图片识别时，`text` 会作为最高优先级的补充说明一起发给模型，用来纠正图片里的叫法。支持「别名 -> 正式名称」写法（也识别 `→` `=>` `=` `就是`）：
>
> ```
> text = "图里说的京东箱子就是纸箱箱子（京东8号->8号纸箱）"
> ```
>
> 效果：图片里的「京东四号」「京东五号」…会按正式名称输出并命中 `4号纸箱`「5号纸箱」…（汉字数字与阿拉伯数字写法等价，如「京东八号」=「京东8号」）。前端 Web 与移动端都会把输入框里的文字自动作为 `text` 传进来。
