...
默认 API 地址也是 `http://***REMOVED-IP***`；如部署了 HTTPS 或更换域名，使用新的地址覆盖 `API_BASE_URL`。Android 当前已允许 HTTP 明文连接，切换 HTTPS 后可移除该兼容设置。
...
默认使用 `***REMOVED-KEYPATH***` 连接 `azureuser@***REMOVED-IP***`。脚本会：合并本地数据库 → 在本地构建 `mobile/dist` → 上传代码和产物 → 在服务器安装 Python 依赖、Nginx（监听 80、托管桌面端和 `/mobile`、反代 API/上传）、systemd 服务并启动。服务器不需要 Node.js。
...
- 不开放注册。账号在项目根目录 **`product_rules.json` 的 `accounts`** 中维护（用于数据库初始化，重启自动同步）。新增业务员示例：
  ```json
  { "username": "xiaowan", ***REMOVED-WEAK-PASSWORD***, "name": "小万", "role": "user" }
  ```
