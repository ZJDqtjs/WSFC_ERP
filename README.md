...
默认使用 `***REMOVED-KEYPATH***` 连接 `azureuser@***REMOVED-IP***`。脚本会：合并本地数据库 → 打包 `app/static/json/data/配置` → 上传到 `/home/azureuser/WSFC_ERP` → 在服务器安装依赖、nginx（监听 80、反代 `/api` `/uploads` 到 127.0.0.1:8000）、systemd 服务并启动。
